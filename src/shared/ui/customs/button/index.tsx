...
const CustomButton = (props: ButtonProps) => {
  return (
    <CustomizedMuiButton
      data-test-id={`custombtn-${props.identifier}`}
    ...
    >
      {props?.loading && (
        <CircularProgress
          data-test-id={`custombtn-loading-${props.identifier}`}
          value={75}
          {...props.loadingprops}
          size={15}
        />
      ...
      {!props?.loading &&
        (props.typography ? (
          <CustomizedTypography data-test-id={`custombtn-text-${props.identifier}`}>
            {props.children}
